...
async def generate_logprobs_and_loss(vllm_client, model, input_ids, target_ids, ignore_index):
...
async def process_single_sample(sample, vllm_client, model, ignore_index, semaphore):
    async with semaphore:
        try:
            chosen_logps, chosen_loss = await generate_logprobs_and_loss(vllm_client, model, sample["chosen_input_ids"].tolist(), sample["chosen_target_ids"].tolist(), ignore_index)
            rejected_logps, rejected_loss = await generate_logprobs_and_loss(vllm_client, model, sample["rejected_input_ids"].tolist(), sample["rejected_target_ids"].tolist(), ignore_index)
...
async def process_file(input_file, output_file, vllm_client, model, ignore_index, concurrency):
    ...
    tasks = [process_single_sample(sample, vllm_client, model, ignore_index, semaphore) for sample in samples]
    ...
    for future in asyncio.as_completed(tasks):
        ...
        if done_count % log_interval == 0 or done_count == total_rows:
            ...
            logger.info(
                ...
                f"ETA: {format_seconds_as_time(eta)}"
            )
...
async def main():
    ...
    vllm_client = AsyncOpenAI(base_url=args.vllm_base_url, api_key=args.vllm_api_key)

    timer = time.time()
    for input_file in input_files:
        output_file = os.path.join(args.output_dir, os.path.basename(input_file))
        results = await process_file(input_file, output_file, vllm_client, args.vllm_model, args.ignore_index, args.concurrency)
        
        save_samples(results, input_file, args)
        stats = calculate_sample_stats(results)
