...
class AllamoDataLoader:
    ...
    def init_datasets(self):
        self.dataset = AllamoDataset(self.config, self.train_split, self.rank, self.world_size)
        self.split_name = 'train' if self.train_split else 'val'
        if not self.dataset.dataset_files:
            raise ValueError(f"No dataset files found for {self.split_name} split")
        logger.info(f"Dataset {self.split_name} split initialized with files: {','.join(self.dataset.dataset_files)}")
        
    def load_datasets(self):
        timer = time.time()
        self.dataset.load_next_dataset()
        dt = time.time() - timer
        logger.info(f"{(len(self.dataset)*self.world_size):,} samples loaded in {dt:.2f} secs")
    ...
    def get_batch(self):
        if self.config.dataset_buffer and self.buffer is not None:
            ...
            samples = self.dataset[self.dataset_offset:self.dataset_offset+self.batch_size]
            self.dataset_offset += self.batch_size
        else:
            samples = []
            for _ in range(self.batch_size):
                if self.dataset_offset >= len(self.dataset):
                    self.reload_dataset(self.dataset)
                samples.append(self.dataset[self.dataset_offset])
                self.dataset_offset += 1
        self.reload_buffer(self.dataset)
    ...
    def reload_dataset(self, dataset):
        ...
        self.dataset_offset = 0
        self.epoch += 1
        logger.info(f"Epoch {self.epoch} finished for {self.split_name} split")
    
    def get_num_loaded_files(self):
        return len(self.dataset.processed_files)
