...
from allamo.fsdp_utils import (
    parallelize_with_fsdp,
)
...
class AllamoFSDPTrainer:
    ...
    def __init_torch(self, config: AllamoConfiguration):
        ...
        init_torch(self.train_ctx, config)
    ...
    def __init_training(self, config: AllamoConfiguration):
        ...
        self.model = parallelize_with_fsdp(model, config, self.fsdp_activation_checkpointing)
    ...
    def forward(self, batch, last_micro_step):
        ...
        if batch["target_weights"] is not None:
            if self.config.weighted_loss_method == 'openchat':
                ...
                loss = (self.train_ctx.world_size / fsdp_loss_weight_acc) * loss
            else:
                loss = loss / torch.sum(batch["target_weights"] > 0).item()
    ...
    def train(self):
        logger.info(f"Starting FSDP training (run id: {self.train_ctx.run_uuid}, world size: {self.train_ctx.world_size}) with configuration: {self.config}")
        ...
        while has_next_iter_to_perform(self.iter_num, self.config, self.data_loader):
            ...
            # timing and logging
            if self.config.log_interval > 0 and self.iter_num % self.config.log_interval == 0 and self.train_ctx.master_process:
                iter_time = time.time() - timer
                # get loss as float. note: this is a CPU-GPU sync point
                lossf = fsdp_loss_acc[0].item() / self.train_ctx.world_size
                ppl = torch.exp(torch.tensor(lossf)).item()
                accuracy = fsdp_loss_acc[2].item() / fsdp_loss_acc[3].item()
                grad_norm = fsdp_loss_acc[4].item() / self.train_ctx.world_size
