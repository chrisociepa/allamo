...
class AllamoSampler:
    ...
    def generate_embeddings(self, text: str, custom_layers_iters: int):
        if text:
            with torch.no_grad():
                with self.ctx:
                    prompt_tokens = self.encode_prompt(text)
                    embeddings = self.model.generate_embeddings(prompt_tokens, custom_layers_iters)
                    embeddings = torch.squeeze(embeddings[:, [-1], :]) # use only the last position
                    return embeddings.tolist()
        return []
                
    def generate_completions(self, text: str, samples: int, new_tokens: int, temperature: float, top_k: int, custom_layers_iters: int):
        result = []
        with torch.no_grad():
            with self.ctx:
                prompt_tokens = self.encode_prompt(text)
                for k in range(samples):
                    y = self.model.generate(prompt_tokens, new_tokens, temperature=temperature, top_k=top_k, custom_layers_iters=custom_layers_iters)
...
if __name__ == '__main__':
    ...
    completions = sampler.generate_completions(config.prompt, config.num_samples, config.max_new_tokens, temperature=config.temperature, top_k=config.top_k, custom_layers_iters=config.layers_iters)
    print("Completions:")
